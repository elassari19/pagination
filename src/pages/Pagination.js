...
const Pagination = ({ state, handelPage }) => {
  ...
  let midel = [...Array(Number(page) <= 2 || Number(page) >= 9 ? 2 : 3)].map(
    (_, idx) =>
      Number(page) == 1 ? (
        <LinkRoute
          handelPage={handelPage}
          pageNumber={Number(page) + 1 + idx}
          page={page}
        />
      ) : Number(page) == 2 ? (
        <LinkRoute
          handelPage={handelPage}
          pageNumber={Number(page) + idx}
          page={page}
        />
      ) : Number(page) == 10 ? (
        <LinkRoute
          handelPage={handelPage}
          pageNumber={Number(page) + idx - 2}
          page={page}
        />
      ) : (
        // :Number(page) == 9
